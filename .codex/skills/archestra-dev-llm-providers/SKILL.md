...
- **Provider message-shape rules**: Gemini and Bedrock require the first non-system turn to be from the user — `ensureLeadingUserTurn` in `prepare-model-messages.ts` prepends one for both (the provider set gating it lives next to the function). Bedrock content rules (every message non-empty, user messages need a text part) are enforced in `normalization/prepare-for-provider.ts` (`ensureBedrockMessageHasContent`, `ensureBedrockUserMessageHasTextPart`); the same file decides per provider whether text documents stay native `document` blocks (Anthropic/Bedrock) or are inlined as decoded text (everyone else).
