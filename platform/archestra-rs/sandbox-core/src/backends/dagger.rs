...
/// pinned to `=0.21.8`. The ordering is load-bearing: take stdout/stderr off the
...
/// `=0.21.8`), so a per-environment host is bound to one child at spawn instead
...
/// recover an engine fault from a *panic* payload. dagger-sdk 0.21.8 `unwrap()`s
    ...
    fn dagger_session_args_match_the_pinned_protocol() {
        assert_eq!(
            ...
            vec![
                ...
                "dagger.io/sdk.name:rust".to_string(),
                "--label".to_string(),
                "dagger.io/sdk.version:0.21.8".to_string(),
            ]
        );
    }
